    ...
    public void moreOptions(LinkedList<Book> ll) {
        ...
        switch (option) {
            case 1: 
                ...
                String add = "add";
                while (add.equals("add")) { //Customer can continue adding as many books as available
                    addCart(ll); //calls the addCart() method with the default catalog of Book objects in the store
                    System.out.println();
                    System.out.println("Any other books you'd like to add?");
                    System.out.print("Type 'add' if yes, anything else if no: ");
                    add = s.nextLine(); //input determines if the while loop continues or not; avoids an infinite loop
                }
                break;
            case 2:
